...
describe('TasksListComponent', () => {
  ...
  beforeEach(async () => {
    mockModalService = jasmine.createSpyObj<ModalService>('ModalService', ['open']);
    allTasksSigna = signal<Task[]>([mockTask]);
    querySignal = signal('');
    filteredTasksSignal = computed(() => {
      return allTasksSigna().filter((task) => {
        return (querySignal().length === 0 ||
    ...
    const mockTaskStore = {
      ...
      markAsCompleted: jasmine.createSpy('markAsCompleted').and.returnValue(of({}))
  ...
  it('should mark task as completed', () => {
    ...
    expect(component.taskStore.markAsCompleted).toHaveBeenCalledWith(mockTask.id);
  });

  it('should open delete task modal with task data', () => {
    component.handleDeleteTask(mockTask);

    expect(mockModalService.open).toHaveBeenCalledWith(DeleteTaskModalComponent, {
      task: mockTask,
    });
  });
  ...
  it('should open update task modal with task data', () => {
    ...
    expect(mockModalService.open).toHaveBeenCalledWith(UpdateTaskModalComponent, {
      task: mockTask,
    });
  });
  ...
  it('should open create task modal', () => {
    ...
    expect(mockModalService.open).toHaveBeenCalledWith(CreateTaskModalComponent);
  });
});
